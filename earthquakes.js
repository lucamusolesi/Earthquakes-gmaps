...
function Bubble(data, options) {
    ...
    if (options) 
		...
		this.options = {
			...
			bubble:{
				fill:{color:"#F00000", opacity:0.6},
				stroke:{color:"#F00000", weight:1, opacity:0.3}
			}
		};
...
Bubble.prototype.drawBubble = function(data, options, row) {
    ...
    var marker = new google.maps.Circle({
        center: new google.maps.LatLng(data.getValue(row, 1), data.getValue(row, 2)),
	...
	var contentString = '<div id="content"><b>UTC Time: </b> ' + data.getValue(row, 0) + ' <b><p> Magnitude:</b> '+sizeOfLocation+' <b><p> Depth:</b> ' + data.getValue(row, 3) + ' Km </div>';

		var infowindow = new google.maps.InfoWindow({
    		content: contentString, 
    		position: new google.maps.LatLng(data.getValue(row, 1), data.getValue(row, 2))
...
Bubble.prototype.drawText = function(data, options, row) {
	...
	if (!textContainer) {
		...
		textItem.innerHTML = data.getValue(row, 0) + ' (' + data.getValue(row, 4) + ')';
    ...
    var position = projection.fromLatLngToDivPixel(new google.maps.LatLng(data.getValue(row, 1), data.getValue(row, 2)));
